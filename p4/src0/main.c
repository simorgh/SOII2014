...
void processDatabase(char** fileList, RBTree * tree, int *nfiles, int start, int end);
...
void* thread_fn(void *arg){
	...
	int start =  args->chunk * (tid);
	int end;
	// obtenim el final de lectura en funció als fitxers restants
	if ( tid == NTHREADS-1 ) end = *(args->nfiles);
	else  end = start + args->chunk;

	printf("\n\t[THREAD %d --CHK_START: %d --CHK_END: %d]", tid, start, end);
	processDatabase(args->fileList, args->tree, args->nfiles, start, end); //Processant base de dades...
...
void processDatabase(char** fileList, RBTree * tree, int *nfiles, int start, int end){
	...
	for(i = start; i < end; i++) {
		// Process file
		hashTable = processFile(fileList[i]);	// processament del fitxer i assignacio de resultats a estructura local
